...
int main() {
    ...
    const fs::path kb = (outdir / R"PET(auto_goal.pkb)PET");
    const fs::path input_file = (outdir / R"PET(auto_goal.paiin)PET");
    const fs::path old_state = (outdir / R"PET(auto_goal_old.pms)PET");
    const fs::path new_kb = (outdir / R"PET(auto_goal_new.pkb)PET");
    const fs::path cycle = (outdir / R"PET(auto_goal.pcycle)PET");
    const fs::path transition = (outdir / R"PET(auto_goal.pkbt)PET");
    const fs::path new_state = (outdir / R"PET(auto_goal_new.pms)PET");
...
rule:risk_inference|hypothesis:fire exists|smoke can indicate fire|fire is dangerous
    ...
    const std::vector<std::string> required = {R"PET(fact:hypothesis:fire exists)PET", R"PET(fact:fire is dangerous)PET", R"PET(fact:avoid fire completed)PET"};
    ...
    std::cout << "petoron_ai_auto_goal_test PASS\n";
    return 0;
}
