...
int main() {
    ...
    const fs::path old_kb = (outdir / R"PET(fake_feedback_old.pkb)PET");
    const fs::path fake_new_kb = (outdir / R"PET(fake_feedback_new.pkb)PET");
    const fs::path cycle = (outdir / R"PET(fake_feedback_cycle.pcycle)PET");
...
relation:causes|fire is hot|hot can burn
    ...
    std::cout << "petoron_ai_kb_update_fake_feedback_test PASS\n";
    return 0;
}
