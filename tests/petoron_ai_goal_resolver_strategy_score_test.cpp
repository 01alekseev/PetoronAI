...
int main() {
    ...
    const fs::path kb = (outdir / R"PET(goal_resolver_strategy_score.pkb)PET");
    const fs::path input_file = (outdir / R"PET(goal_resolver_strategy_score.paiin)PET");
...
rule:risk_inference|storm is dangerous|fire is dangerous|danger exists
    ...
    std::cout << "petoron_ai_goal_resolver_strategy_score_test PASS\n";
    return 0;
}
